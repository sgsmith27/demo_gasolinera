@extends('layouts.app', ['title' => 'Estado de cuenta'])
...
<div class="grid gap-4 md:gap-6">
    <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-semibold text-slate-800">Estado de cuenta</h1>
                <p class="text-sm text-slate-500">Cliente: {{ $customer->name }}</p>
            </div>

            <a href="/customers/{{ $customer->id }}/statement/pdf"
               target="_blank"
        ...
        <div class="overflow-x-auto rounded-xl border border-slate-200">
            <table class="min-w-[1100px] w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-4 text-slate-500">No hay documentos para este cliente.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
