...
<div class="grid gap-4 md:gap-6">
    <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
        <div class="mb-4">
            <h1 class="text-xl font-semibold text-slate-800">Cuentas por cobrar</h1>
            <p class="text-sm text-slate-500">Ventas registradas al crédito.</p>
        </div>

        <form method="GET" action="/accounts-receivable" class="grid grid-cols-1 md:grid-cols-5 gap-3 items-end mb-5">
            <label class="text-sm">
            ...
            <label class="text-sm">
                <span class="block mb-1 text-slate-600">Cliente</span>
                <select name="customer_id" class="w-full border border-slate-300 rounded-xl px-3 py-2">
            ...
            <div class="flex gap-2">
                ...
                <a href="/accounts-receivable/pdf?from={{ $from }}&to={{ $to }}&customer_id={{ $customerId }}&status={{ $status }}"
                   target="_blank"
            ...
            <table class="min-w-[1200px] w-full text-left text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 font-semibold text-slate-600">Cliente</th>
                        <th class="px-4 py-3 font-semibold text-slate-600">Venta</th>
                        <th class="px-4 py-3 font-semibold text-slate-600">Monto original</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($accounts as $account)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3">#{{ $account->sale_id }}</td>
                            ...
                            <td class="px-4 py-3 flex gap-3">
                                <a href="/accounts-receivable/{{ $account->id }}" class="text-indigo-600 hover:underline">Ver detalle</a>
                                <a href="/customers/{{ $account->customer_id }}/statement" class="text-slate-700 hover:underline">Estado de cuenta</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-4 text-slate-500">No hay cuentas por cobrar registradas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
