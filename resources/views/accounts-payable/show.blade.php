@extends('layouts.app', ['title' => 'Detalle cuenta por pagar'])
...
<div class="grid gap-4 md:gap-6">
    ...
    <div>
        <h1 class="text-xl font-semibold text-slate-800">Cuenta por pagar #{{ $accountPayable->id }}</h1>
        <p class="text-sm text-slate-500">Proveedor: {{ $accountPayable->supplier?->name ?? '—' }}</p>
    </div>
    <a href="/accounts-payable/{{ $accountPayable->id }}/pdf"
       target="_blank"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
            ...
            <div class="text-2xl font-bold mt-2">Q{{ number_format((float)$accountPayable->original_amount_q, 2) }}</div>
        </div>

        <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
            <div class="text-sm text-slate-500">Pagado</div>
            <div class="text-2xl font-bold mt-2">Q{{ number_format((float)$accountPayable->paid_amount_q, 2) }}</div>
        </div>

        <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
            <div class="text-sm text-slate-500">Saldo pendiente</div>
            <div class="text-2xl font-bold mt-2">Q{{ number_format((float)$accountPayable->balance_q, 2) }}</div>
        </div>

        <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
            <div class="text-sm text-slate-500">Estado</div>
            <div class="text-2xl font-bold mt-2">{{ strtoupper($accountPayable->status) }}</div>
        </div>
    </div>

    @if($accountPayable->status !== 'paid')
        <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
            <h2 class="text-lg font-semibold text-slate-800 mb-4">Registrar pago</h2>
            ...
            <form method="POST" action="/accounts-payable/{{ $accountPayable->id }}/payments" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                @csrf
    ...
    <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
        <h2 class="text-lg font-semibold text-slate-800 mb-4">Historial de pagos</h2>
        ...
        <div class="overflow-x-auto rounded-xl border border-slate-200">
            <table class="min-w-[800px] w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-4 text-slate-500">No hay pagos registrados.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
